...
data Element = Element
  {element :: [TileType], rest :: [TileType]}
  deriving (Eq, Show)
...
extractRun :: [TileType] -> TileType -> [Element]
extractRun ts East = []
extractRun ts South = []
extractRun ts West = []
extractRun ts North = []
  ...
  if t `notElem` ts
    then []
      ...
      let mm2 = if t == C1 || t == D1 || t == B1 || t == C2 || t == D2 || t == B2 then Nothing else safeToEnum @TileType (fromEnum t - 2)
          ...
          mk t1 t2 t3 ts' = if includeElement t1 t2 t3 ts' then Just Element {rest = deleteElement t1 t2 t3 ts', element = [t1, t2, t3]} else Nothing
          pos1 = do
            m2 <- mm2
            m1 <- mm1
            mk m2 m1 t ts
...
extractRunForAll :: [TileType] -> [Element]
extractRunForAll rs =
  nub (concatMap (extractRun rs) [C1, C2, C3, C4, C5, C6, C7, C8, C9, D1, D2, D3, D4, D5, D6, D7, D8, D9, B1, B2, B3, B4, B5, B6, B7, B8, B9])

concatExt :: Element -> Element -> Element
concatExt e1 e2 = Element {}

recur :: Element -> [Element]
recur ex = extractRunForAll (rest ex)

extRec :: [TileType] -> Bool
extRec ts = True
